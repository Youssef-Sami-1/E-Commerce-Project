...
export default async function Home() {
  ...
  return (
    <div className="space-y-12" suppressHydrationWarning>
      ...
      <section className="relative h-[70vh] md:h-[80vh] w-full overflow-hidden animate-fade-in reveal-up" data-anim suppressHydrationWarning>
        <Image
        ...
        <div className="relative z-10 max-w-6xl mx-auto h-full px-4 flex items-end md:items-center">
          <div className="pb-10 md:pb-0 load-stagger">
            ...
            <div className="mt-6 flex flex-col sm:flex-row gap-3">
              <Link href="/products" className="px-5 py-2.5 rounded-full bg-slate-900 text-white hover:shadow-[0_0_30px_rgba(0,0,0,0.2)] transition-all text-center">Shop Products</Link>
              <Link href="/categories" className="px-5 py-2.5 rounded-full border border-slate-300 bg-white/80 hover:bg-white transition-all text-center">Browse Categories</Link>
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="max-w-7xl mx-auto px-4 space-y-12">
        <section className="reveal-up" suppressHydrationWarning>
          <div className="flex items-center justify-between mb-4 reveal-up" suppressHydrationWarning>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 stagger reveal-up" suppressHydrationWarning>
            {categories.slice(0, 8).map((c: { _id: string; name: string; image: string }) => (
        ...
        <section className="reveal-up" suppressHydrationWarning>
          ...
          <div className="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5 stagger reveal-up" suppressHydrationWarning>
            {products.slice(0, 12).map((p: { _id: string; title: string; price: number; imageCover: string }) => (
